...
if parse.mat=='manual':
    ...
    mat = np.matmul(arr1, np.linalg.inv(arr2))
    udst = cv2.warpAffine(img, mat[:2], (img.shape[0], img.shape[1]))
    cv2.imwrite('../results/undistorted_manual_4.jpg',udst)
    cv2.imshow('frame', udst)
    cv2.waitKey(0)

# 2. API
elif parse.mat=='api':
    mat = cv2.cv2.getperspectivetransform(pts1, pts2)
    udst = cv2.warpperspective(img, mat, (img.shape[0], img.shape[1]))
